...
function removeFile()
	...
	if ngx.var.request_method ~= "OPTIONS"  then
		...
		if (ngx.var.request_method ~= "POST"  or ngx.req.get_headers()["token"] == nil or ngx.req.get_headers()["token"] == "") then
		...
		--判断是否传入了后端回调程序路径
			...
			if(os.execute("rm -rf /Users/daizhiqiang/upload_store/"..params["path"]) == 0) then
